...
import { OrgMemberDialog } from "./OrgMemberDialog"
...
export function OrgMemberTable({ members, users, roles, orgs, onRefresh }: Props) {
    ...
    const filtered = members.filter(m => {
        ...
        const matchStatus = !filters.status || m.roleId === filters.status
        const matchFrom = !filters.dateFrom || new Date(m.joinedAt) >= new Date(filters.dateFrom)
        const matchTo = !filters.dateTo || new Date(m.joinedAt) <= new Date(filters.dateTo)
        ...
        <div className="space-y-4">
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-sm text-muted-foreground">สมาชิกใน Organization</p>
                </div>
                {canWrite && (
                        ...
                        {filtered.length === 0 ? (
                            ...
                            <TableRow key={m.id}>
                                ...
                                <TableCell className="text-sm text-muted-foreground">
                                    {format(new Date(m.joinedAt), "d MMM yyyy", { locale: th })}
                                </TableCell>
                                <TableCell>
                                    <div className="flex items-center gap-1">
                                        {canWrite && (
                                            <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => { setSelected(m); setDialogOpen(true) }}>
                                                <Pencil className="h-4 w-4" />
                                            </Button>
                                        )}
                                        {canDelete && (
                                            <Button variant="ghost" size="icon" className="h-8 w-8 text-destructive hover:text-destructive" onClick={() => { setSelected(m); setDeleteOpen(true) }}>
            ...
            <OrgMemberDialog open={dialogOpen} onOpenChange={setDialogOpen} member={selected} users={users} roles={roles} orgs={orgs} onSuccess={onRefresh} />
                ...
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            นำ &quot;{selected?.user.name ?? selected?.user.email}&quot; ออกจาก &quot;{selected?.org.name}&quot;?
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
